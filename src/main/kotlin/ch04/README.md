...
이는 캡슐화를 위반했기 때문이며, Screening이 노출된 구현에 직접적으로 의존하기 때문이다. 각 객체 안에 존재해아할 로직이 밖으로 새어 나왔다.
...
이 데이터를 이용하는 절차는 별도 객체 안에 구현된다. 

이러면 접근자와 수정자를 사용해야 하니 캡슐화가 무너진다.

데이터를 처리하는 작업과 데이터를 같은 객체 안에 두더라도 데이터를 오퍼레이션보다 먼저 결정하게 된다면 인터페이스에 해당 정보가 드러난다.

이로인해 캡슐화에 실패하면 객체의 구현이 인터페이스를 어지럽히고 응집도와 결합도에 나쁜 영향을 미쳐 변경에 취약한 코드를 만든다.

### 데이터 중심 설계는 객체를 고립시킨 채 오퍼레이션을 정의하도록 만든다.

객체지향 설계의 무게 중심은 객체의 외부에 맞춰져 있어야 한다. 어떤 상태를 가지고 어떻게 관리할지는 부가적인 문제다.

데이터 중심 설계는 객체 내부에 초점을 맞춘다. 실행 문맥에 대한 고민 없이 세부 정보가 결정된다.

이미 구현된 객체 끼리 협력 방법을 고민하기 때문에 인터페이스를 억지로 끼워맞춰야 한다.

두 번째 설계에서 변경에 유연하게 대처하지 못했던 이유다. 객체의 인터페이스에 구현이 노출돼있으면 협력하는 객체 모두가 영향을 받는다.
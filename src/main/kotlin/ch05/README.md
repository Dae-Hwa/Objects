...
책임주도 설계에 익숙해지는데는 경험과 학습이 필요하다. 그래도 어렵다.

최대한 빠르게 목적한 기능을 수행하는 코드를 작성한 뒤 책임을 올바른 위치로 이동시키는 방법도 있다. - 리팩토링

캡슐화를 향상시키고 응집도를 높이고 결합도를 낮추되, 동작은 그대로 유지해야 한다.

처음에 적당한 위치를 찾지 못하는 경우 리팩토링

### 메서드 응집도

[ReservationAgency.kt](../ch04/v01/ReservationAgency.kt) 의 `reserve` 메소드

길고 이해하기 어렵다. 만약에 이것보다 더 길다면?
- 어떤일을 수행하는지 한눈에 파악하기 힘들어서 코드 파악에 시간이 걸린다
- 하나의 메서드 안에서 너무 많은 작업을 처리한다. -> 수정해야 할 부분 찾기 힘듦
- 메소드 내부의 일부 로직만 수정하더라도 나머지 부분에서 버그 발생 가능성이 높다
- 로직 일부만 재사용하는 것이 불가능
- 코드 재사용하라면 복분해야 하니 중복 초래

긴 메소드는 응집도가 낮고 이해하기도 어렵고 재사용하기도 어렵고 변경하기도 어렵다. 이런 메서드를 몬스터 메소드라 한다.

메소드가 명령문들의 그룹으로 구성되고 각 그룹에 주석을 달아야 한다면 그 메소드의 응집도가 낮은 것

메서드를 잘게 분해하면 주석 없이 메서드의 응집도 높일 수 있음

응집도가 높은 메소드의 변경 이유는 단 하나여야 한다.  클래스가 작고 목적이 명확한 메소드들로 구성되어 있어야 한다.
- 어떤 메서드를 수정해야 하는지 판단하기 쉬움
- 재사용 용이
- 작은 메소드들로 조합된 메소드는 주석을 나열한 것 같이 보임 -> 코드 이해하기 쉬워짐

메소드 이름은 짧고 이해하기 쉬운게 좋다
- 메소드가 잘 나뉘어지면? 다른 곳에서 사용될 확률 높아짐
- 고수준의 메소드를 보면 주석을 읽는 것 같은 느낌이 들게 할 수 있음
- 잘게 나뉘어지면 오버라이딩하는 것도 쉬움
- 메소드 길이가 보다는 이름과 몸체의 의미적 차이가 중요

객체 책임 분배시 메소드를 으집도 있는 수준으로 분해하는게 가장 먼저 해야 할 일

긴 메서드를 작고 응집성 높게 분리하면 이동하기 편해지기 때문

[메소드 분리 ReservationAgency](v04/p1_%EB%A9%94%EC%86%8C%EB%93%9C%EB%B6%84%EB%A6%AC/ReservationAgency.kt)

각각의 메소드는 하나의 작업만 수행한다. 변경의 이유가 하나이며 작고 명확하고 응집도가 높음

클래스는 더 길어졌지만 클래스 길이보다 명확성이 더 중요. 이러면 변경하기 쉬운 코드가 됨

수정 후의 메소드를 보면 상위 수준의 명세를 읽는 것 같은 느낌. 어떤 일을 하는지 한 눈에 파악 가능함

수정 후에는 변경하기도 더 쉬워짐. 변경사항이 생기면 특정 메소드만 고치면 됨

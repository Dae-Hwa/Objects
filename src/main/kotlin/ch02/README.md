...
다형성을 이용해야 할때는 상속을 이해야 한다.

>> 갑자기 생각나서 메모. 객체지향의 여러 파트를 나눈다면? 현실세계와 비슷한 모델링을 해서 이해하기 쉽게 한다.
>> 변경하기 쉽게 만들어 유연하게 만든다.
>> 그런데 변경하기 쉽게만 만들면 가독성이 떨어진다. 그러면 이해하기 힘들어진다. 그래서 그 사이 트레이드오프를 잘 조율해야 한다.
